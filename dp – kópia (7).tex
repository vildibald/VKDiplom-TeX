...
Zvyšné hodnoty derivácií toti môeme s malou odchılkou vypoèíta napríklad De Boorovov interpoláciou.
...
\begin{oznac}
	...
	\end{itemize}
\end{oznac}
Na to aby sme zostrojili splajnové segmenty pochopite¾ne potrebujeme ma dané všetky uzly aj funkèné hodnoty v uzloch a derivácie v uzloch.
Analogicky ako pri krivkách, ak nemáme známe všetky derivácie, vieme napriek tomu zvyšné derivácie dopoèíta De Boorovov interpoláciou s ve¾mi malou odchılkou aby splajn bol spojitı a hladkı.
Na to aby sme zostrojili splajn pomocou De Boorovho modelu interpolácie potrebujeme ma známe
\begin{itemize}
	\item
...
De Boor navrhol vhodnı model, ktorı okrem hodnôt $z_{i,j}$ na mrieke ve¾kosti $I \times J$ vyaduje zadanie smerovıch derivácií na okrajoch mrieky uzlov a štyri zmiešané derivácie v rohoch.
...
Splajny sa taktie dajú poui na interpoláciu matematickıch funkcií. 
\begin{df}
...
\end{itemize}
\end{df}
Na splajn interpolujúci funkciu $f$ môeme aplikova De Boorovu interpoláciu a zo získanıch derivácií u vieme zostroji jeho segmenty.
...
Teraz si ukáeme ako môeme pod¾a zadanıch uzlov, ich funkènıch hodnôt a derivácií zostroji segmenty splajnu ako funkcie dvoch premennıch v priestore.
...
\end{lema}
Tento modifikovanı algoritmus má menej ne polovièné pamäové nároky ne v prípade všeobecnıch trojdiagonálnych sústava èo nám v praxi umoòuje rieši väèšie úlohy. 
...
Redukovanı algoritmus De Boorovej interpolácie obsahuje väèší poèet aritmetickıch operácií ako sú sèítanie, násobenie a delenie.  
...
Následne pri poèítaní rıchlosti algoritmov budeme schopní k jednotlivım krokom algoritmu a aritmetickım operáciám v jednotlivıch krokoch priradi \pojem{váhy}, ktoré urèia cenu tıchto krokov a z nich vypoèítame celkové ceny algoritmov a následne zrıchlenie redukovaného spôsobu.
...
Obrázok \ref{img:cacheSpeed} názorne ukazuje vzah medzi ve¾kosou dátovej štruktúry v KiB a poètom strojovıch cyklov, ktoré procesor musí èaka pre prístup k jej prvkom. 
...
\item
Hodnota $\delta^\rightY$ znaèí \pojem{pomer rıchlosti kopírovania a násobenia} pri operáciách s plávajúcou desatinnou èiarkou a spojitıch dátach.
\item
Hodnota $\delta^\righttherefore$ znaèí \pojem{pomer rıchlosti kopírovania a násobenia} pri operáciách s plávajúcou desatinnou èiarkou a nespojitıch dátach.
\end{itemize}
\end{oznac}
\begin{pozn}
	Pre moderné procesory architektúry x86 budeme uvaova $\beta = 2$, $\gamma = 3$, $\delta^\rightY = \rfrac{1}{25}$ a $\delta^\righttherefore = \rfrac{3}{5}$.
\end{pozn}
\begin{pozn}
...
\begin{table}
	\begin{center}
		\begin{tabular}{lcccc}			
			...
			\midrule
			\textit{VypoèítajDx}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\delta^\rightY IJ$\\
			\textit{VypoèítajDxy}
			& $8I$ & $6I$ & $2\gamma I$ & $\delta^\rightY I$\\
			\textit{VypoèítajDy}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\delta^\rightY IJ$\\
			\textit{VypoèítajDyx}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\delta^\rightY IJ$\\
...
$3\gamma IJ$ delení,
\item
$3\delta^\rightY IJ$ kopírovaní.
\end{itemize}
Cena plného algoritmu teda je
$$21IJ+3\gamma IJ+3\delta^\rightY IJ\textit{.}$$
\end{lema}
...
\begin{center}
	\begin{tabular}{lcccc}	
		...
		\textit{VyriešLU}
		& $3K$ & $2K$ & $\gamma K$ & $\delta^\righttherefore K$\\
		\bottomrule
	\end{tabular}
\end{center}
...
\begin{center}
	...
	\begin{tabular}{lcccc}	
		...
		\midrule
		\textit{VypoèítajDx}
		& $\rfrac{3}{2}IJ+\rfrac{3}{2\beta}IJ$ & $1IJ+\rfrac{1}{\beta}IJ$ & $\rfrac{1}{2}\gamma IJ$ & $\rfrac{1}{2}\delta^\righttherefore IJ$\\
		\textit{VypoèítajDy}
		& $\rfrac{3}{2}IJ+\rfrac{3}{2\beta}IJ$ & $1IJ+\rfrac{1}{\beta}IJ$ & $\rfrac{1}{2}\gamma IJ$ & $\rfrac{1}{2}\delta^\righttherefore IJ$\\
		\textit{VypoèítajDxy}
		& $8I+8J$ & $6I+6J$ & $2I+2J$ & $\delta^\rightY I+\delta^\rightY J$\\
		\textit{VypoèítajDyx}
		& $\rfrac{3}{4}IJ+\rfrac{33}{4\beta}IJ$ & $\rfrac{1}{2}IJ+\rfrac{17}{4\beta}IJ$ & $\rfrac{1}{4}\gamma IJ$ & $\rfrac{1}{2}\delta^\righttherefore $\\
		\bottomrule
	\end{tabular}
\end{center}	
...
\begin{lema}
	...
	\begin{itemize} 
		...
		\item
		$\rfrac{3}{2}\delta^\righttherefore IJ$ kopírovaní.
	\end{itemize}
	Cena redukovaného algoritmu teda je
	$$\rfrac{25}{4}IJ + \rfrac{57}{2\beta}IJ + \rfrac{5}{4}\cdot \gamma IJ + \rfrac{3}{2}\delta^\righttherefore IJ\textit{.}$$
\end{lema}
...
$$\frac{21IJ+3\gamma IJ+3\delta^\rightY IJ}
	{\rfrac{25}{4}IJ + \rfrac{57}{2\beta}IJ + \rfrac{5}{4}\cdot \gamma IJ + \rfrac{3}{2}\delta^\righttherefore IJ}\textit{.}$$ 
\end{veta}
...
\pr{ \label{pr:ratio1}
	...
	Ïalej nech $\delta^\rightY = \rfrac{1}{25}$ a $\delta^\righttherefore = \rfrac{3}{5}$. 
