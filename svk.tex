...
Témou mojej diplomovej práce sú priestorové splajn povrchy, prièom naším cie¾om je preskúma nové poznatky o splajnoch, na ktorıch istı èas pracuje vedúci tejto práce doc. RNDr. Csaba Török, CSc.

Vısledok tejto práce je návrh novej metódy vıpoètu derivácií splajnu v jeho uzloch, jej porovnanie s de Boorovym postupom z ktorého vychádzame a vysvetlenie zrıchlenia našej metódy.
...
%	Hovoríme, e splajn $S$ interpoluje funkciu $f$ ak pre kadé $i$ z $\{0, \dots, I-1\}$ a pre kadé $j$ z $\{0, \dots, J-1\}$ platí $z_{i,j} = f(u_i, v_j)$. 
%%	\begin{itemize}
%%		\item
%%		$z_{i,j} = f(u_i, v_j)$ nazveme \pojem{funkèné hodnoty splajnu v uzloch}.
%%		\item
%%		$d^x_{i,j} \approx \pd{f(u_i, v_j)}{x}$ nazveme \pojem{(smerové) $x$-ové derivácie v uzloch}.
%%		\item
%%		$d^y_{i,j} = \pd{f(u_i, v_j)}{y}$ nazveme \pojem{(smerové) $y$-ové derivácie v uzloch}.
%%		\item
%%		$d^{xy}_{i,j} = \pdd{f(u_i, v_j)}{x}{y}$ nazveme \pojem{zmiešané derivácie v uzloch}.
%%	\end{itemize}
%\end{df}
...
Algoritmus môeme teda ešte ïalej priestorovo a navyše aj vıpoètovo optimalizova,pretoe oboch prípadoch všetky èísla na hornej aj spodnej diagonále majú hodnotu $1$. 
...
Pouitı prekladaè bol Intel \cpp\ Compiler v 64 bitovej verzii nastavenı na generovanie agresívne optimalizovaného binárneho kódu (-O2).    
...
\begin{center}
	\begin{tabular}{lcccc}
		\toprule
		& \multicolumn{2}{c}{Krivka} & \multicolumn{2}{c}{Sériovo}  \\
		Procesor
		& Plnı & Reduk. & Plnı & Reduk. \\
		\midrule
		FX-6300 \tiny{3C/6T}
		&  &  &  & \\
		C2D E8200 \tiny{2C/2T}
