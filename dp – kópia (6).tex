...
Pre úplnos skôr ne si povieme o splajnoch, predstavíme si pojem polynomickej funkcie ktorı je neoddelite¾nou súèasou pri definícii splajnu. 
...
Najbenejšie kubické splajny triedy $C^2$, teda tie pri ktorıch v uzloch máme zaruèenú rovnos aj derivácií druhého rádu, sú naturálne a clamped splajny. Pre naše potreby budeme uvaova iba druhé menované prièom ich budeme konštruova pouitím hermitovskıch bázovıch funkcíí (pozri èas \ref{sec:her_constr}).  
...
\begin{df}\label{df:curve_spline}
	...
	Nech pre kadé $i$ z $\{0, 1, \dots I-2\}$ funkcie $S_{i}$ sú polynomické funkcie, ktoré spåòajú
	\begin{itemize}
		\item
...
Na to aby sme zostrojili splajn potrebujeme ma známe uzly $(u_0, \dots,u_{I-1})$, funkèné hodnoty $(z_0, \dots,z_{I-1})$ a dve krajné derivácie $d_0$ a $d_{I-1}$. 
...
Nech pre kadé $i$ z $\{0, 1, \dots I-2\}$ a $j$ z $\{0, 1, \dots J-2\}$ funkcie $S_{i,j}$ sú bipolynomické funkcie, ktoré spåòajú
\begin{itemize}
\item
...
Kım v prípade splajnovıch kriviek bolo evidentné hneï od zaèiatky ich vıskumu a aplikácie, e okrem hodnôt $(z_0, \dots,z_{I-1})$ je potrebné zada ešte dve podmienky, v našom prípade sú to dve hodnoty $d_0$ a $d_{I-1}$.
V prípade interpolaènıch splajnovıch povrchoch nebolo zrejmé ktoré smerové a zmiešané derivácie je potrebné zada.
De Boor navrhol vhodnı model, ktorı okrem hodnôt $z_{i,j}$ na mrieke ve¾kosti $I \times J$ vyaduje zadanie smerovıch derivácií na okrajoch mrieky a štyri zmiešané derivácie v rohoch.
...
Ak uvaujeme ¾ubovo¾nú funkciu tak pomocou nej vieme nájs iadané hodnoty derivácií. 
Navyše takto získanı splajn bude danú funkciu interpolova, teda ju \pojem{napodobní}. 
...
Poskytujú prunı nástroj na modelovanie reálnych situácií na lokálne poiadavky, prièom ich vıpoèet je rıchly a stabilnı. 
...
Priebené zhrnutie práce je úloha na základe vstupnıch uzlov $u_{0}$, ..., $u_{I-1}$, $v_{0}$, ..., $v_{J-1}$ a funkènıch hodnôt $z_{0,0}$, ..., $z_{I,J}$ nájs \pojem{hladkú}, po èastiach definovanú funkciu $S: [u_0, u_{I+1}] \times [v_0, v_{J+1}] \to \mathbb{R}$ so spojitımi deriváciami prvého aj druhého rádu takú, e pre kadé $i \in  {0, \dots, I-1}$ a $j \in  {0, \dots, J-1}$ platí $z_{i,j} = S(u_i, v_j)$. Funkèné hodnoty a 
%a hodnoty derivácií $S$ v uzloch sú èo najblišie hodnotám derivácií $f$. 
...
Hodnoty $z_{i,j}$ máme vopred dané. Derivácie $d^x_{i,j}$, $d^y_{i,j}$ a $d^{xy}_{i,j}$ buï môu by dané všetky, alebo iba niektoré a z nich vieme s malou odchılkou odhadnú hodnoty ostatnıch. 
...
Základnım kameòom De Boorovej interpolácie tkvie v opakovanom poèítaní systémov trojdiagonálnych lineárnych rovníc. 
...
Jeden z asymptoticky efektívnych spôsobov riešenia tıchto rovníc spoèíva v $LU$ dekompozícii $A\mathbf{x} = L\underbrace{U\mathbf{x}}_{\mathbf{y}} = \mathbf{r}$:
...
Procedúra algoritmu je priestorovo optimalizovaná kde pouívame LU rozklad robíme priamo na vstupnıch vektoroch predstavujúce diagonály a vısledok ukladáme do vektora obsahujúci pravú stranu.
\newpage
\begin{algorithm}
...
Tento modifikovanı algoritmus má menej ne polovièné pamäové nároky ne v prípade všeobecnıch trojdiagonálnych sústava èo nám v praxi to umoòuje rieši väèšie úlohy. 
Navyše pri reálnej implementácií nemusíme vektory $r$ a $p$ inicializova pri kadom volaní procedúry \pojem{VyriešLU} ale namiesto toho môeme poui prednaèítané vyrovnávacie vektory, tzv. \pojem{buffer}.
...
Nech sú dané uzly $(u_0, \dots, u_{I-1})$ a $(v_0, \dots, v_{J-1})$, kde $I$,$J$ sú z \uinteger, prièom chceme interpolova funkèné hodnoty $(z_{0,0}, \dots, z_{0,J-1}, \dots, z_{I-1,0}, \dots, z_{I-1, J-1}$. 
Vıslednı splajn bude teda tvorenı $(I-1) \cdot (J-1)$ segmentami. 
Ako bolo spomenuté, kadı segment splajnu potrebuje štyri uzly a hodnoty $z$, $d^x$, $d^y$ a $d^{xy}$, ktoré potrebujeme získa z funkcie $f$. 
Získanie derivácií môe by niekedy nákladné. 
Príkladom môe by v prípad, keï funkèné hodnoty $z_{i,j}$ sú získané vyhodnotením nejakej funkcie $f$.
To môe by v praxi na poèítaèi znaène nákladné najmä v prípade, ak pracujeme so symbolicky zapísanou funkciou vo forme textového reazca, ktorú je potrebné dynamicky interpretova poèas behu programu. 

Algoritmus nájdenı Carlom de Boorom \cite{deboor} umoòuje s malou odchılkou vypoèíta hodnoty derivácií v uzloch na základe následujúcich vstupnıch hodnôt, ktoré máme dané, napríklad vyhodnotením funkcie $f$:
...
Podstatou je vısledok vıskumu \cite{szabominotorok} doc. Töröka a RNDr. Szaba o vzahu medzi bikvartickımi polynómami a bikubickımi splajnami a odvodenie sústav na základe tohto vısledku \cite{minoTorok}.
...
\pozn{
	...
	Preto v tejto èasti budeme naïalej uvaova iba rovnomerne rastúce postupnosti uzlov.
}
...
%\subsubsection{Paralelizácia}
%\paragraph{Paralelizácia}
\subparagraph{Paralelizácia}
Všimnime si, e v prípade oboch algoritmoch jednotlivé derivácie poèítame v vdy jednom smere.
...
Analogická úvaha platí aj pre $y$-ové aj zmiešané derivácie. 


\section{Konštrukcia hermitovho splajnu}
\label{sec:her_constr}
Teraz si ukáeme ako pod¾a zadanıch uzlov a vypoèítanıch derivácií zostroji segmenty splajnu ako funkcie dvoch premennıch v priestore.
...
\begin{lema}
	...
	Nech sú splnené podmienky na zaèiatku tejto èasti (\ref{sec:her_constr}.
...
Pri urèení asymptotickej èasovej zloitosti zvyèajne zanedbávame rıchlosti jednoduchıch krokov algoritmu ako sú napríklad aritmetické operácie, porovnávanie ve¾kosti èísel, kopírovanie a podobne.
Keï porovnávame rıchlos asymptoticky rovnako rıchlych postupoch musíme bra do úvahy vplyvy jednotlivıch tıchto triviálnych krokov. 
...
Redukovanı algoritmus De Boorovej interpolácie obsahuje väèší poèet aritmetickıch operácií ako sú sèítanie, násobenie, delenie.  
V èasti \ref{sec:theory_speedup} si ukáeme, e redukovanı algoritmus je skutoène rıchlejší. 
Tu iba poznamenajme, e zrıchlenie nastáva vïaka zredukovanému poètu delení.
V tejto kapitole si spoèítame poèty tıchto operácií, ale predtım popíšeme technické parametre procesorov v dôsledku ktorıch dosahujeme zrıchlenie. 
Následne pri poèítaní rıchlosti algoritmov budeme schopní k jednotlivım krokom algoritmu a aritmetickım operáciám v jednotlivıch krokoch priradi \pojem{váhy}.
Potom u budeme schopní presne urèi skutoèné zrıchlenie a porovna ho so zrıchlením v konkrétnej implementácii. 
...
Toti ak jeden prvok matice má $8$ bajtov (ve¾kos èíselného typu \textit{double} vo väèšine jazykoch), tak matica zaberá v pamäti pribline $7\text{,}63$ megabajtov. 
Pri matici typu pole polí máme $1000$ polí kadé zaberajúce práve $7\text{,}81$ kilobajtov ($1$ kB = $1024$ B, $1$ MB = $1024$ kB, atï.). 
...
Obrázok \ref{img:cacheSpeed} názorne ukazuje vzah medzi ve¾kosou dátovej štruktúry v kB a poètom strojovıch cyklov, ktoré procesor musí èaka pre prístup k jej prvkom. 
...
Narozdiel od celıch jadier pri vláknovom paralelizme, tieto jednotky vo všeobecnosti nedokáu fungova samostatne a simultánne spracováva viacero vlákien alebo procesov\footnote{Existujú technológie ako napríklad Hyper-threading umoòujúce za urèitıch podmienok uplatni vláknovı paralelizmus aj v rámci jediného superskalárneho jadra.}. 
...
Vektorizácia funguje ak vykonávame operácie na vektore tak, e vıpoèet $i$-teho prvku nezávisí na vısledku vıpoètu (napr.) $(i-1)$-teho prvku.
V algoritme \ref{alg:SolveLU} pre LU dekompozíciu môeme vidie, e toto neplatí. 
Vektorizácia nás teda nemusí zaujíma.

Pipelining je zaloenı na myšlienke princípu fungovania vıpoètovıch jednotiek podobne ako vırobná linka vo fabrike, kde novı vırobok na linku vstúpi skôr ako je predchádzajúci dokonèenı. 
...
\pozn{
	...
	Ale aj po zanedbaní dostávame korešpondujúce vısledky meraní a vypoèítaného zrıchlenia, ktoré sa líšia o najviac 10\%.
...
Na obrázku \ref{obr:skylakeCore} vidíme schému jedného jadra procesorovej mikroarchitektúry Intel Skylake, ktorá je ku dòu písania práce najmodernejšou bene dostupnou procesorovou generáciou a na ktorej testujeme náš zrıchlenie redukovaného algoritmu. 
...
Pointa superskalárnosti v tomto prípade spoèíva v monosti vıpoèet vırazov typu $a_0\circ a_1\circ\dots\circ a_n$, kde $\circ \in \{+, \cdot\}$ rozloi medzi dve jednotky a dosiahnu dvojnásobného zrıchlenia. 
Toto má znaènı vplyv na rıchlos najmä redukovaného algoritmu pri poèítaní pravıch strán trojdiagonálnych rovníc, ktoré sú na prvı poh¾ad zloitejšie ako v prípade plného algoritmu. 
...
V situácií kedy jeden proces (vlákno) nemôe vyui všetky jednotky jadra, niektoré procesory umoòujú nevyuité porty priradi inému procesu (vláknu). 
...
Procesory AMD majú implementovanı odlišnı variant tejto techniky zvanı \pojem{Clustered Multi-Threading}, kde namiesto klasickıch jadier je procesor vybavenı tzv. modulmi, kde kadı modul má dve samostatné \uv{oklieštené} jadrá schopné spracováva iba celoèíselné operácie a jedinú FPU jednotku spoloènú pre oba jadrá. 
...
V opaènom prípade dochádza k javu kedy jeden proces (vlákno) musí èaka na uvo¾nenie jednotky druhám procesom (vláknom) a vtedy môe dôjs k zníeniu vıkonu oproti identickému procesoru s neaktívnym SMT.
...
\begin{center}
 	...
 	\end{tabular}
\end{center}
\caption{Tabu¾ka aritmetickıch operácií na rôznych mikroarchitektúrach.}
...
Pre zaujímavos si môeme ukáza praktické vısledky z testovacej aplikácie. 
...
Ïalej budeme mera aj pamäovú operáciu kopírovania, prièom rozlišujeme medzi kopírovaním spojitıch a nespojitıch dát.
...
\begin{center}
	\begin{tabular}{lccccc}	
		\toprule
		Procesor
		& $\pm$ & $\cdot$ & $\div$ & $\rightY$ & $\righttherefore$\\
		\midrule
		AMD FX-6300
		& 176 & 173 & 673 & &  \\
	...
	\end{tabular}
\end{center}
\caption{Rıchlos aritmetickıch operácií na konkrétnych CPU.}
\label{real_op_speed}	
\end{table}
...
Z toho dôvodu môeme inverznı prietok v prípade delenia zanedba a v kontexte implementovanıch algoritmov delenie povaova pod¾a jeho odozvy za pribline \textbf{trojnásobne} pomalšie ako násobenie. 
...
\begin{df}
\label{df:oper_count}
Nech $\mathcal{V}$ je matematickı vıraz obsahujúci $s$ sèítaní, $n$ násobení a $d$ delení. 
Definujme funkciu $o: \mathbb{V} \to \mathbb{N} \times \mathbb{N} \times \mathbb{N}$ vzahom 
$$o(\mathcal{V}) = \langle s, n, d \rangle\text{.}$$
Funkciu $o$ budeme nazıva \pojem{poèet operácií vırazu $V$}.
\end{df}
...
\begin{itemize}
\item
Hodnota $\beta$ znaèí \pojem{faktor superskalárnosti} operácií s plávajúcou desatinnou èiarkou.
Inımi slovami hodnota $\beta$ znaèí poèet jednotiek jadra procesora schopnıch poèíta desatinné èísla.
\item
Hodnota $\gamma$ znaèí \pojem{pomer odozvy delenia a násobenia} pri operáciách s plávajúcou desatinnou èiarkou.
Inımi slovami hodnota $\gamma$ znaèí ko¾kokrát je delenie pomalšie oproti sèítaniu v prípade, keï nedochádza k èastému opakovanému násobeniu (resp. sèítaniu).
\end{itemize}
\end{oznac}

\begin{pozn}
	Pre moderné procesory architektúry x86 budeme uvaova $\beta = 2$ a $\gamma = 3$.
\end{pozn}
	
\begin{pozn}
Faktor superskalárnosti $\beta$ budeme uvaova iba pre operácie sèítania a násobenia. 
iadny bene dostupnı procesor toti nedokáe paralelizova delenie.
\end{pozn}
...
\begin{df}
	...
	Nech $s$ je poèet sèítaní, $n$ je poèet násobení a $d$ je poèet delení.
	Definujme funkcie cien operácií
	\begin{itemize}
		\item 
		$c: \mathbb{N} \times \mathbb{N} \times \mathbb{N} \to \mathbb{N} \times \mathbb{N} \times \mathbb{N}$ 
		vzahom 
		$$c(\langle s, n, d \rangle) = \left\langle \ceil*{\frac{s}{\beta}}, \ceil*{\frac{s}{\beta}}, \gamma d \right\rangle\text{,}$$
		ktorú nazveme \pojem{cena operácií}.
		\item 
		Nech $V$ je matematickı vıraz a $o(V) = \langle s, n, d \rangle$. 
		Potom \pojem{cena operácií vırazu $V$} je funkcia
		$c: \mathbb{V} \to \mathbb{N} \times \mathbb{N} \times \mathbb{N}$ 
		v tvare
...
\begin{oznac}
	Struène doplòme poèty aj pre procedúry a algoritmy.
...
\begin{oznac}
	...
	Analogicky oznaème aj ceny procedúr a algoritmov.
\begin{itemize}
	\item 
	Nech $\mathcal{P}$ je procedúra. Funkciu
...
To budeme poèíta tak, e si zadefinujeme \uv{procedúry} ktoré predstavujú jednotlivé èasti algoritmu pod¾a èastí \ref{sec:tridiagonal}, \ref{sec:full_deboor} a \ref{sec:red_deboor}. 

%Keïe všetky procesory pouité pri meraní obsahujú dve jednotky pre operácie s plávajúcou desatinnou èiarkou tak pri poèítaní budeme uvaova, e pri vırazoch s viacerımi operáciami vezmeme do úvahy len poloviènı poèet operácií. 
%Poèty operácií v tabu¾kách sú u po aplikovaní cien.
Najprv polome procedúry predstavujúce LU dekompozíciu spoloèné pre oba postupy.
\begin{itemize}
	\item
	Procedúra \textit{InicalizujLU} inicializuje hodnoty pravej strany sústavy rovníc $r_0$, \dots, $r_{K-1}$ a hodnotu $b$ z LU dekompozície \ref{eq:LU} pod¾a \ref{eq:deboorM} v prípade plného algoritmu, respektíve pod¾a \ref{eq:redM} v prípade redukovaného algoritmu.
	\item
...
\label{sec:full_cost}
Poèty základnıch aritmetickıch a pamäovıch operácií pre vyššie uvedené procedúry môeme zhrnú do tabu¾ky, prièom uvaujeme poèty v našej ukákovej implementácii\footnote{Repozitár so zdrojovımi kódmi k aplikáciám mono nájs na adrese https://github.com/vildibald/VKDiplom-master}.
...
Operácia $\rightY$ predstavuje kopírovanie vypoèítanıch derivácií do vıslednej matice uzlov. 
Pri meraní sa ukázalo, e kopírovanie spojitıch dát vieme má cenu $0\text{,}04$ teda kopírovanie je $25$ násobne rıchlejšie ako sèítanie.
V tabu¾kách budeme pre sèítanie, násobenie a delenie uvádza ich ceny pod¾a definície \ref{df:oper_weights}. 
V prípade plného algoritmu dochádza len ku kopírovaniu spojitıch dát, teda poèet kopírovaní v tabu¾kách vynásobime $0\text{,}04$.
...
Keïe poèty rastú kvadraticky tak pre ve¾ké $I$ a $J$ bude odchılka zanedbate¾ná. 
...
\begin{table}
	\begin{center}
		\begin{tabular}{lcccc}			
			...
			\midrule
			\textit{VypoèítajDx}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\rfrac{1}{25}IJ$\\
			\textit{VypoèítajDxy}
			& $8I$ & $6I$ & $2\gamma I$ & $\rfrac{1}{25}I$\\
			\textit{VypoèítajDy}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\rfrac{1}{25}IJ$\\
			\textit{VypoèítajDyx}
			& $4IJ$ & $3IJ$ & $\gamma IJ$ & $\rfrac{1}{25}IJ$\\
...
$3\gamma IJ$ delení,
\item
$0\rfrac{3}{25}IJ$ kopírovaní.
\end{itemize}
Cena plného algoritmu teda je
$$\rfrac{528}{25}IJ+3\gamma IJ\textit{.}$$
\end{lema}
...
Cena kopírovania bude pod¾a reálneho merania $0.6$.
...
\begin{center}
	\begin{tabular}{lcccc}	
		\toprule
		Procedúra
		& $\pm$ & $\cdot$ & $\div$ &  $\righttherefore$ \\
		\midrule
		\textit{InicalizujLU}
		& $\rfrac{3}{\beta}K$ & $\rfrac{2}{\beta}K$ & $0$ & $0$\\
		\textit{VyriešLU}
		& $3K$ & $2K$ & $\gamma K$ & $\rfrac{3}{5}K$\\
		\bottomrule
	\end{tabular}
\end{center}
...
Moderné procesory architektúry x86 obsahujú práve dve jednotky pre vıpoèty s pohyblivou desatinnou èiarkou èo znamená $\beta = 2$.
...
Osobitne si spoèítajme ceny aj pre tieto procedúry.
...
%		\textit{VyriešZmiešLU}
%		& $16\textit{,}5K$ & $8\textit{,}5K$ & $0$ & $K$\\
%		\bottomrule
%	\end{tabular}
%\end{center}
%\caption{Poèty operácií LU dekompozície na nespojitıch dátach pre zmiešané zostatkové derivácie.}
%\label{cross_lu_operations_uncached}	
%\end{table}

Analogicky ako v predchádzajúcej sekcii polome procedúry predstavujúce implementáciu redukovaného algoritmu pod¾a èasti \ref{sec:red_deboor}::
\begin{itemize}
	\item
...
\begin{center}
	...
	\begin{tabular}{lcccc}	
		...
		\midrule
		\textit{VypoèítajDx}
		& $\rfrac{3}{2}IJ+\rfrac{3}{2\beta}IJ$ & $1IJ+\rfrac{1}{\beta}IJ$ & $\rfrac{1}{2}\gamma IJ$ & $\rfrac{3}{10}IJ$\\
		\textit{VypoèítajDy}
		& $\rfrac{3}{2}IJ+\rfrac{3}{2\beta}IJ$ & $1IJ+\rfrac{1}{\beta}IJ$ & $\rfrac{1}{2}\gamma IJ$ & $\rfrac{3}{10}IJ$\\
		\textit{VypoèítajDxy}
		& $8I+8J$ & $6I+6J$ & $2I+2J$ & $I+J$\\
		\textit{VypoèítajDyx}
		& $\rfrac{3}{4}IJ+\rfrac{33}{4\beta}IJ$ & $\rfrac{1}{2}IJ+\rfrac{17}{4\beta}IJ$ & $\rfrac{1}{4}\gamma IJ$ & $\rfrac{3}{25}IJ$\\
		\bottomrule
	\end{tabular}
\end{center}	
...
%		& $0$ & $0$ & $0$ & $0\textit{,}5IJ$\\
%		\textit{VypoèítajDyx}
%		& $4\textit{,}125IJ$ & $2\textit{,}125IJ$ & $0$ & $0\textit{,}25IJ$\\	
%		%\textbf{\textit{VypoèítajRedukovanı}}
%		%& $4\textit{,}125IJ$ & $2\textit{,}125IJ$ & $0$ & $1\textit{,}25IJ$\\
%		\bottomrule
%	\end{tabular}
%\end{center}	
%\caption{Poèty operácií na nespojitıch dátach redukovaného algoritmu.}
%\label{red_operations_uncached}
%\end{table}
...
\begin{lema}
	...
	Nech $I$ a $J$ oznaèujú poèty uzlov na osiach $x$ a $y$. 
	Potom sumárny poèet operácií plného algoritmu je
	\begin{itemize} 
		\item
		...
		\item
		$0\text{,}75IJ$ kopírovaní.
	\end{itemize}
	Cena redukovaného algoritmu teda je
	$$7IJ + \rfrac{57}{2\beta}IJ + \rfrac{5}{4}\gamma IJ\textit{.}$$
\end{lema}
...
Po spoèítaní jednotlivıch matematickıch a pamäovıch operácií v èastiach \ref{sec:full_cost} a \ref{sec:red_cost} pristúpime k formulácii zrıchlenia.


\begin{veta}
\label{veta:full_red_ratio}
Nech $I$ a $J$ oznaèujú poèty uzlov na osiach $x$ a $y$.
Potom zrıchlenie redukovaného algoritmu je
$$\frac{\rfrac{528}{25}IJ+3\gamma IJ}
	{7IJ + \rfrac{57}{2\beta}IJ + \rfrac{5}{4}\gamma IJ}\textit{.}$$ 
\end{veta}

Ukáme si dva príklady zrıchlenia aby sme názorne videli vplyv superskalárnosti na rıchlos redukovaného algoritmu.
V prvom príklade predpokladajme porovnanie algoritmov na primitívnom procesore. 
...
\pr{ \label{pr:ratio1}
	...
	\ref{img:cacheSpeed}), $\iota = 1\textit{,},1$ a $\lambda = 1$. 
	Dosadením do vzahu z predchádzajúcej vety dostaneme
	$$\frac{30\textit{,}12}{36\textit{,}75} \approx 0\textit{,}77\textit{.}$$
	Pomer je menší ako 1 èo znamená, e bez superskalárnosti ($\beta = 1$) je redukovanı algoritmus pomalší ako plnı.
}
...
\pr{ \label{pr:ratio2}
	...
	$$\frac{30\textit{,}12}{25} \approx 1\textit{,}2\textit{.}$$
	V prípade superskalárnosti s faktorom 2 je redukovanı algoritmus o 20\% rıchlejší ako plnı.
}
...
Je otázne nako¾ko by s rastúcim faktorom  $\beta$ rástlo zrıchlenie.
Keïe nemáme k dispozícii procesor s vhodnou hardvérovou vıbavou (ak vôbec takı existuje) je nároèné predpoklada ako by si takıto stroj s redukovanım postupom poradil. 
...
Pouitı prekladaè bol Intel \cpp\ Compiler v 64 bitovej verzii nastavenı na generovanie agresívne optimalizovaného binárneho kódu (-O2), prièom paralelnı kód bol implementovanı pomocou rozhrania OpenMP.    
...
Ostávajúce ståpce analogicky znázoròujú èasy a pomer paralelnıch verzií implementovanıch pod¾a myšlienky uvedenej na konci èasti \ref{sec:red_deboor}. Všetky uvedené èasové údaje sú v milisekundách.  
\begin{table}
\begin{center}
	...
	\end{tabular}
\end{center}
\caption{Reálne merania plného a redukovaného algoritmu na mrieke $1000\times 1000$ uzlov.}
\label{real_speedup}
\end{table}
...
Sériová verzia redukovaného algoritmu je o pribline 20\% rıchlejšia ako pôvodnı de Boorov postup èo súhlasí s odhadnutım zlepšením na konci kapitoly \nameref{spdup} v príklade \ref{ratio2}. 
...
Môeme všimnú malé škálovanie vıkonu paralelizovaného algoritmu najmä v prípade procesorov obsahujúcich nejakú formu SMT kde je redukovanı algoritmus dokonca o nieèo pomalší ako plnı. 
...
Máme rozpracované zefektívnenie vıpoètu zmiešanıch restov, ktoré sa nám ale ešte nepodarilo do práce dokonèi. 

V prípade SMT procesorov sa dá pomalos, aj keï to nemáme overené, vysvetli samotnou superskalárnosou pod¾a podsekcie \ref{sec:instrukcny_paralelizmus}. 
...
Pri redukovanom algoritme ale máme ve¾a operácií v tvare $a_0a_1\circ \dots\circ a_n$, kde $\circ \in \{+, \cdot\}$ kde optimalizujúci prekladaè môe takıto vıpoèet rozloi do viacerıch strojovıch inštrukcií. 
V tomto prípade dôjde k vyuitiu oboch FPU jadra v rámci jediného vlákna ale napriek tomu SMT na toto jadro priradí aj druhé vlákno ktoré poèíta práve na FPU. 
Dôjde tak k zbytoènej réii plánovaèa vlákien èi u na úrovni OS alebo inštrukèného plánovaèa samotného procesora, èo spôsobí pokles vıkonu. 
...
Samotnı vıvoj aplikácii môe prebieha v ¾ubovo¾nom programovacom jazyku beiacom pod virtuálnym strojom Common Language Runtime. 
...
Pretoe CLR znemoòuje korektne porovnáva procesorovı èas a pamäové nároky implementovanıch algoritmov, rozhodli sme sa vıkonnostnı tester oddeli do samostatnej aplikácie implementovanej v natívnom jazyku, konkrétne v jazyku \cpp. 
...
\obrazok{apka_vrch.png}{0.3}{Lišta nástrojov.\label{apka_vrch}}
...
Teraz si popíšme druhú implementovanú aplikáciu zameranú nie na vykres¾ovanie, ale na ukáku èi skutoène je redukovanı algoritmus rıchlejší.  
\obrazok{bench.png}{0.277}{Vıkonnostnı tester.\label{bench}}
\obrazok{bench_surface.png}{0.277}{Zadanie údajov pre testovanie.\label{bench_surface}}
\obrazok{bench_surface_results.png}{0.277}{Vısledky testu.\label{bench_surface_results}}
Na obrázku \ref{bench} môeme vidie úvodné okno aplikácie, ktoré obsahuje sedem poloiek. 
Ich vıznam si teraz vyjasníme.
 \begin{itemize}
 	...
 	Pred spustením aplikácie vyaduje postupne zadanie dvoj údajov. 
 	...
 	Menší poèet dokáe väèšina modernıch poèítaèov vypoèíta príliš rıchlo aby ten èas dokázala aplikácia rozumne zmera.  
 	\item[3.]
...
Podarilo sa nám v prípade rovnomerne rozloenıch uzlov urıchli vıpoèet derivácií splajnov v uzloch a tie zmenši pamäovú nároènos vıpoètu na polovicu. 
Napriek tomu, e vıslednı redukovanı algoritmus obsahuje viac aritmetickıch operácií je vïaka povahe tıchto operácií rıchlejší o pribline 20\%.
Prioritou teraz ostáva jednak zovšeobecni redukovanı algoritmus aj pre splajny s nerovnomernımi uzlami a upravi zmiešané zbytkové derivácie aby sme mohli zefektívni paralelizáciu.
%Zlepšenie paralelizácie by v budúcnosti umonilo vıpoèty akcelerova grafickou kartou. 
Paralelné poèítanie uzlov redukovanım algoritmom nie je v súèasnosti rıchlejšie vo všeobecnosti, ale závisí na konkrétnej procesorovej architektúre.
Vıhodou ale ostávajú polovièné pamäové nároky èo umoòuje rieši väèšie úlohy a tak rozdeli prácu medzi väèší poèet logickıch procesorov. 
