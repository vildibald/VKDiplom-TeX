...
\typprace{Analıza a návrh riešenia}
\rok{2015}
\miesto{Košice}
%\podakovanie{
...
Vısledkom tejto práce je okrem návrhu tejto novej metódy vıpoètu aj porovnanie so známymi metódami a implementácia aplikácie na vizuálne porovnávanie jednotlivıch metód.
...
Vıhodou tohto frameworku je, kede beí na platforme Microsoft .NET, monos jednoduchej portácie na desktopovú prípadne mobilnú aplikáciu. Štruktúra práce je nasledovná.
\begin{itemize}
\item{\textbf{Splajn}}
...
\item{\textbf{Novı postup generovania bikubickıch splajnov}}
...
\begin{itemize}
	...
	\item
	Analıza modelov interpolaènıch splajn-povrchov a podpory paralelného programovania. .NET Framework
	\item
	Algoritmizácia, paralelizáciaa vizualizácia splajn-povrchov.
	\item
	Implementácia paralelnıch algoritmov vybranıch modelov splajn-povrchov pomocou viacvláknovıch API pre multiprocesorové systémy a ich porovnanie.
\end{itemize}

\section*{Splajn}

V našej práci pracujeme s hermitovskımi splajnami, ktoré sú štandardne triedy $C^1$, teda splajnami, ktorıch prvé derivácie v uzloch sa rovnajú. V našej práci však budeme uvaova hermitovské splajny triedy $C^2$, teda splajny pri ktorıch sa v uzloch rovnajú navyše aj derivácie druhého rádu.
...
\begin{itemize}
	...
	\item
	$\Delta x = u_i - u_{i-1}$ a $\Delta y = v_j - v_{j-1}$ pre ¾ubovo¾né $i \in  \{1, \dots, I+1\}$, resp. $j \in  \{1, \dots, J+1\}$.
\end{itemize}
}

Úlohou je na základe vstupnıch uzlov nájs \pojem{hladkú}, po èastiach definovanú funkciu $F: [u_0, u_{I+1}] \times [v_0, v_{J+1}] \to \mathbb{R}$ z so spojitımi deriváciami prvého a druhého rádu takú, e pre kadé $i \in  {0, \dots, I+1}$ a $j \in  {0, \dots, J+1}$ platí $F(u_i, v_j) = z_{i,j}$. Funkcia $F$ predstavuje splajn, prièom jej jednotlivé èasti nazveme segmenty. 
...
\begin{df}
	...
	\begin{itemize}
		...
		\item
		Nech pre kadé $i$ z $\{0, \dots, I\}$ a $j$ z $\{0, \dots, J\}$ je $S_{i,j}$ bikubická polynomická funkcia z  2-rozmerného intervalu $[u_{i}, u_{i+1}] \times [v_{j}, v_{j+1}]$ do \real.
	\end{itemize}
	%\newpage
...
Ak $i$ je z  $\{0, \dots, I\}$ a $j$ je z $\{0, \dots, J\}$, tak štvorice uzlov $(u_i, v_j)$, $(u_i, v_{j+1})$, $(u_{i+1}, v_j)$ a $(u_{i+1}, v_{j+1}$ tvoria obdånikovı úsek nad ktorım sa nachádza splajnovı segment. Kadı segment $S_{i,j}$ je bikubická funkcia z $[u_i, u_{i+1}] \times [v_j, v_{j+1}]$ do $\mathbb{R}$. Vısledná funkcia $F$ teda vznikne zjednotením segmentov $S_{i,j}$. Na vygenerovanie kadého segmentu potrebujeme štyri uzly, a pre kadı uzol príslušne hodnoty $z$, $dx$, $dy$ a $dxy$. Tieto hodnoty vieme získa buï priamo z interpolovanej funkcie $f$, alebo ich vypoèíta inım spôsobom.



\section*{De Boorova interpolácia}

Nech sú dané dané hodnoty $(u_0$, ...,$u_{I+1})$ a $(v_0$, ...,$v_{J+1})$, kde $I$,$J$ sú z \uinteger, prièom chceme interpolova funkciu $S$ na $[u_0, u_I] \times [v_0, v_J]$. Vıslednı splajn bude tvorenı $(I+1) \cdot (J+1)$ segmentami. Ako bolo spomenuté, kadı segment potrebuje štyri uzly a hodnoty $z$, $dx$, $dy$ a $dxy$. Pri de Boorovej interpolácii nám treba pozna tieto hodnoty:
\begin{itemize}
	\item
...
Na príklade uzlov z obrázka \ref{pic:fseg} potrebujeme pozna hodnoty takto:
\begin{itemize}
	\item
...
Zvyšné hodnoty $z$, $dx$, $dy$ a $dxy$ vieme jednoznaène vypoèíta pomocou $2(I+1) + J + 6$ lineárnych sústav s celkovo $3(I+1)(J+1) + I + J + 2$ rovnicami:
...
Tento splajn jednoznaène interpoluje funkciu $S$.

\pozn{
De Boorova interpolácia vo všeobecnosti nepredpokladá len rovnomerne rastúce postupnosti $(u_0$, ...,$u_I)$ a $(v_0$, ...,$v_J)$. Náš postup v ïalšej èasti èlánku ale funguje len s takımito postupnosami. Preto sme v tejto èasti popísali len špeciálny prípad interpolácie pre takto urèené uzly.
}

\section*{Novı postup generovania bikubickıch splajnov}

V rámci svojej bakalárskej práce som popisoval tento postup pre kubické splajnové kriovky triedy C2, teda splajny, kde interpolovaná funkcia $f$ je typu $\mathbb{R} \to \mathbb{R}$. Jednım z cie¾om tejto práce je postup zovšeobecni pre bikubické splajny, teda pre splajny, kde interpolovaná funkcia $f$ je typu $\mathbb{R} \times \mathbb{R} \to \mathbb{R}$.

Nišie popíšeme redukovanı systém pod¾a doc. Töröka pouívanı pre kubické splajny. Neznáme $D_0$, ..., $D_{K+1}$ sú h¾adané hodnoty prvıch derivácií a $Y_0$, ..., $Y_{K+1}$, v prípade kubického splajnu, predstavujú funkèné hodnoty interpolovanej funkcie $f$ v uzloch $u_0$ a $u_{K+1}$.
...
Ako pri de Boorovi tak aj pri našom spôsobe generovania derivácií dostávame nieko¾ko systémov trojdiagonálnych lineárnych rovníc. Štandardnı spôsob riešenia takıchto rovníc
...
Porovnajme si poèet operácií násobenia pri oboch postupoch. Podotıkam, e toto konkrétne porovnanie sa teraz tıka len pri kubickıch splajnoch. LU dekompozícia a spätnı prechod obsahujú operáciu delenia, ktorá je v tabu¾ke nišie oznaèená $\gamma$. Symbol $\gamma$ udáva, e jedna operácia delenia má cenu práve $\gamma$ násobení. Reálna hodnota závisí od procesora, prièom bıva rovná pribline 3.

Môeme vidie, e vıpoèet neznámych redukovanım algoritmom je rıchlejší ne de Boorov.
Zrıchlenie je $\frac{4+4\gamma}{5+2\gamma} \approx 1{,}45$, prièom hodnoty $d_k$, pre nepárne $k$ (\ref{eq:reducedOddD}) môu by poèítané paralelne.

\begin{table}[h] 
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{llllllll}
			& (\ref{eq:LU})  & (\ref{eq:deboorM}), (\ref{eq:reducedL}) & (\ref{eq:FwLy}) & (\ref{eq:BwUd}) & (\ref{eq:reducedR})  &  &  \\ \hline
			\multicolumn{1}{|l|}{{\bf Spôsob}} & \multicolumn{1}{l|}{{\bf LU}} & \multicolumn{1}{l|}{{\bf Matica}} & \multicolumn{1}{l|}{{\bf LU priamy}} & \multicolumn{1}{l|}{{\bf LU spätnı}} & \multicolumn{1}{l|}{{\bf Zbytok}} & \multicolumn{1}{l|}{{\bf Celkovo}} & \multicolumn{1}{l|}{{\bf Zrıchlenie}} \\ \hline
			de Boor & $\gamma (K-1)$ & $1+K$ & $K-1$ & $\gamma K$ &  & $2(1+\gamma)K-\gamma$ & \\
			Redukovanı & $\gamma \left(\frac{K}{2}-1\right)$ & $2+2\frac{K}{2}$ & $\frac{K}{2}-1$ & $\gamma \frac{K}{2}$ & $1+2\frac{K}{2}$ & $\left(\frac{5}{2}+\gamma\right) K+2-\gamma$ & $\frac{4+4\gamma}{5+2\gamma}$ 
		\end{tabular}
	}
	\caption{Poèet násobení s zrıchlenie pre K párne}
\end{table}


\section*{Implementácia}

Naším cie¾om je vytvorenie webovej aplikácie na báze Microsoft Silverlight(ïalej len Silverlight). Silverlight je bezplatnı nástroj na tvorbu webovıch aplikácií spustite¾nıch priamo vo webovom prehliadaèi. 
...
\pozn{
	...
	Prípadná portácia na GNU/Linux by musela by vo forme offline desktopovej aplikácie.
}

Silverlight ponúka hardvérovo akcelerované API podporujúce tvorbu 3D grafickıch aplikácií a hier. V dobe písania èlánku je grafickı engine a funkènı návrh uívate¾ského rozhrania hotovı. Pracujem teraz na samotnej funkcionalite, ako je vizuálne odlíšenie rozdielov medzi splajnami, benchmarku generovania splajnov a rôznych dodatoènıch funkcionalít. 

\obrazok{apka.png}{0.7}{Ukáka z aplikácie}

\section*{Záver}

Ciele tejto práce sme splnili zatia¾ na cca 30\%, prièom doteraz som sa hlavne venoval implementaènej èasti. Poèas letnıch prázdnin sa hodlám pusti do hlavného cie¾a, èím je rozšírenie nového postupu z kriviek na splajnové plochy. Pri kubickıch krivkách sa novı postup ukázal funkènı a rıchlejší ako predchádzajúci spôsoby. V súèasnosti je webová aplikácia takmer hotová, prièom sa  z tejto práce, prièom sa dolaïujú rôzne detaily a vylepšuje sa ovládanie uívate¾ského rozhrania. Predpokladáme, e finálna verzia by mala by hotová v priebehu júla 2015. 
...
\bibitem{2}  I. Szabó, L. Miòo, C. Török. Biquarticpolynomials in bicubicspline construction, PF UPJŠ, 2014 
\bibitem{3}  L. Miòo: Parametrické modelovanie dát komplexnej štruktúry, PF UPJŠ, 2014
