...
\begin{document}
	...
	Úloha De Boorovej splajnovej interpolácie sa definuje vstupnımi údajmi, pozri obrázok \ref{obr:deboor}.
	...
	\end{itemize}
	\obrazok{DeBoorModel.png}{0.7}{De Boorov model vstupnıch hodnôt pre splajnovı povrch.\label{obr:deboor}}
	...
	Je známe, e Hermitov krivkovı aj povrchovı splajn je triedy $C^1$, teda derivácie prvého rádu v uzloch sú spojité. 
	...
	Témou našej práce je úloha na základe vstupnıch uzlov $u_{0}$, ..., $u_{I-1}$, $v_{0}$, ..., $v_{J-1}$ a funkènıch hodnôt $z_{0,0}$, ..., $z_{I,J}$ nájs \pojem{hladkú}, po èastiach definovanú funkciu $S: [u_0, u_{I+1}] \times [v_0, v_{J+1}] \to \mathbb{R}$ so spojitımi deriváciami prvého aj druhého rádu takú, e pre kadé $i \in  {0, \dots, I-1}$ a $j \in  {0, \dots, J-1}$ platí $z_{i,j} = S(u_i, v_j)$. 
	...
	Cie¾om tejto práce je analyzova a vylepši De Boorov algoritmus pre bikubické splajny na základe vısledku vıskumu \cite{szabominotorok}  RNDr. Szaba a doc. Töröka o vzahu medzi bikvartickımi polynómami a bikubickımi splajnami a odvodenie sústav na základe tohto vısledku \cite{minoTorok}.
	...
	V tejto èasti popíšeme rıchlos aritmetickıvh operácií na základe meraní a dokumentácie vırobcov procesorov.\\
	...
	Na oba algoritmy ale vektorizácia nemá vplyv.
	...
	V tabu¾ke \ref{tab:instr_speed} uvidíme rıchlosti štyroch základnıch matematickıch operácií v rámci šiestich testovanıch mikroarchitektúr. 
	...
	\footnotetext{\label{footnote:round_divisor}
		Odozva je nadobúda menšie hodnoty ak je delite¾ celı.}
	...
	Na základe našich testov môeme konštatova e sèítavanie a násobenie môeme povaova za podobne rıchle operácie. 
	...
	\pozn{
		V následujúcich èastiach budeme pre pomer rıchlosti delenia a sèítania namiesto údajov z dokumentácie uvaova nami namerané vısledky, ktoré lepšie korešpondujú s meranım aj teoretickım zrıchlením redoukovaného algoritmu. 
	}
	...
	\begin{df}
		...
		Definujme funkciu $o: \mathbb{V} \to \mathbb{N}^3$ vzahom 
	...
	\label{sec:full_cost}
	Ceny základnıch aritmetickıch a pamäovıch operácií pre vyššie uvedené procedúry môeme zhrnú do tabu¾ky \ref{lu_operations}, prièom uvaujeme poèty v našej ukákovej implementácii\footnote{Repozitár so zdrojovımi kódmi k aplikáciám mono nájs na adrese https://github.com/vildibald/VKDiplom-master}.
	...
	Sèítaním všetkıch cien operácií v tabu¾ke dostaneme nasledujúci vısledok.
	...
	V èasti \ref{sec:spup} si ukáeme, e redukovanı algoritmus je v dôsledku vplyvu inštrukèného paralelizmu a ceny za operácie delenia skutoène rıchlejší. 
	...
	\begin{veta}
		...
		$$\frac{21+3\gamma}
		{\rfrac{25}{4} + \rfrac{57}{2\beta} + \rfrac{5}{4}\cdot \gamma}\text{,}$$
		kde $\beta$ je faktor inštrukèného paralelizmu a $\gamma$ je pomer rıchlosti delenia a sèítania. 
	\end{veta}
	...
	\pr{ \label{pr:ratio2}
		...
		$$\frac{30}{24\textit{,}25} \approx 1\textit{,}17\textit{.}$$
		V prípade inštrukèného paralelizmu s faktorom 2 je redukovanı algoritmus o 23\% rıchlejší ako plnı.
	}
	...
	\pr{ \label{pr:ratio1}
		...
		$$\frac{30}{38\textit{,}5} \approx 0\textit{,}78\textit{.}$$
	...
	\end{table}
	V tabu¾ke \ref{real_vs_theory_speedup} budeme v ståpci 2 uvaova pomery nameranıch rıchlosti delenia a sèítania operácií z tabu¾ky \ref{real_op_speed} v èasti \ref{sec:instr_rychlost}. 
	Ståpec 3 predstavuje teoretické zrıchlenie redukovaného algoritmu vypoèítané pod¾a vety \ref{veta:full_red_ratio} v predchádzajúcej èasti.
	Poslednı ståpec 4 predstavuje merané zrıchlenie sériovej verzie redukovaného algoritmu pod¾a predchádzajúcej tabu¾ky.
	Ako vidie, namerané zrıchlenie algoritmu korešponduje s teoretickım zrıchlením v rámci malej odchılky. 
	...
	\begin{thebibliography}{9}
		...
		\bibitem{minoTorok} L. Miòo, C. Török, Fast algorithm for spline surfaces, Communication of the Joint Institute for Nuclear Research, Dubna, 2015, E11-2015-77
