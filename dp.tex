...
\abstract{
	...
	This work aims to analyze recently derived algorithm for bicubic clamped interpolation in three-dimensional space, design it's effective implementation and find out reasons of faster calculations of this new approach.
}
...
\begin{itemize}
	...
	Ukáka modifikovaného postupu pre kubické splajny a jeho rozšírenie pre bikubické splajny.	
%	\item{\textbf{Konštrukcia splajnu}}\newline
%	Zostrojenie splajnu ako zloenej funkcie segmentov.
\end{itemize}
...
\begin{itemize}
	...
	Spoèítanie èasovej zloitosti  vysvetlenie vplyvu základnıch aritmetickıch a pamäovıch operácií. 
...
Najbenejšie kubické splajny triedy $C^2$, teda tie pri ktorıch v uzloch máme zaruèenú rovnos aj derivácií druhého rádu, sú \pojem{naturálne} a \pojem{clamped} splajny. Pre naše potreby budeme uvaova iba druhé menované prièom ich budeme konštruova pouitím Hermitovıch bázovıch funkcíí (pozri èas \ref{sec:her_constr}).%, prièom ich budeme struène nazıva \pojem{hermitovské splajny}.  
...
\begin{df}\label{df:curve_spline}
	Nech $I \ge 0$ je prirodzené èíslo, $(u_0, \dots,u_{I-1})$ je rastúca postupnos reálnych èísel.
	Nech pre kadé $i$ z $\{0, 1, \dots I-2\}$ funkcie $S_{i}$ sú polynomické funkcie premennej $x$, ktoré spåòajú
	\begin{itemize}
		\item
	...
	\end{cases}
	\end{equation}
	nazveme \pojem{krivkovı clamped splajn triedy $C^1$ na uzloch $(u_0, \dots,u_{I-1})$} alebo skrátene.
\end{df}
...
Nech pre kadé $i$ z $\{0, 1, \dots, I-2\}$ a $j$ z $\{0, 1, \dots, J-2\}$ funkcie $S_{i,j}$ sú bipolynomické funkcie premennıch $x$ a $y$, ktoré spåòajú
\begin{itemize}
\item
...
Na zostrojenie splajnu pomocou De Boorovho modelu interpolácie potrebujeme ma známe nasledujúce vstupné údaje. (pozri aj obrázok \ref{pic:deboor_model})
\begin{itemize}
	\item
	...
	\item
	zmiešané derivácie $\{d^{xy}_{0,0}, d^{xy}_{I-1,0}, d^{xy}_{0,J-1}, d^{xy}_{I-1,J-1}\}$ v rohoch mrieky.
\end{itemize}
\obrazok{DeBoorModel.png}{0.7}{De Boorov model vstupnıch hodnôt pre splajnovı povrch.\label{pic:deboor_model}}
...
De Boor navrhol vhodnı model, ktorı okrem hodnôt $z_{i,j}$ na mrieke ve¾kosti $I \times J$ vyaduje zadanie smerovıch derivácií na okrajoch mrieky uzlov a štyri zmiešané derivácie v rohoch (pozri vyššie).

Zdôrazòujeme, e takto definovanı krivkovı aj povrchovı splajn je triedy $C^1$. 
Pri vhodne zvolenıch hodnotách derivácií v uzloch vieme ale zaruèi rovnos aj druhıch derivácií, èim dostaneme splajn z triedy $C^2$ èo nám zaruèí vyššiu hladkos \pojem{spojenia} jednotlivıch segmentov. 
...
\begin{itemize}
	...
	\item
	$d^{xy}_{i,j} = \pdd{f(u_i, v_j)}{x}{y}$, pre kadé $i$ z $\{0, I-1\}$ a pre kadé $j$ z $\{0, J-1\}$.
\end{itemize}
\end{df}
...
Takto získanı splajn bude danú funkciu interpolova, teda ju \pojem{napodobní} (ako na ilustraènom obrázku \ref{pic:mexhat}). 
...
\obrazok{mexHatWKnots.png}{0.7}{Funkcia $sin(\sqrt{x^2+y^2})$ interpolovaná bikubickım splajnom.\label{pic:mexhat}}
...
V kadom uzle predpokladáme zadané
\begin{itemize}
	\item
...
%\end{lema}
Ak $i$ je z $\{0, \dots, I-2\}$ a $j$ je z $\{0, \dots, J-2\}$, tak štvorice uzlov $(u_i, v_j)$, $(u_i, v_{j+1})$, $(u_{i+1}, v_j)$ a $(u_{i+1}, v_{j+1})$ tvoria obdånikovı úsek nad ktorım sa nachádza splajnovı segment. Kadı segment $S_{i,j}$ je bikubická funkcia z $[u_i, u_{i+1}] \times [v_j, v_{j+1}]$ do $\mathbb{R}$. 
Vısledná funkcia $S$ teda vznikne zjednotením segmentov $S_{i,j}$. 
Na vypoèítanie kadého segmentu potrebujeme štyri uzly, a pre kadı uzol príslušne hodnoty $z_{i,j}$, $d^x_{i,j}$, $d^y_{i,j}$ a $d^{xy}_{i,j}$. 

\pozn{
Hodnoty $z_{i,j}$ musíme ma vdy vopred dané. Derivácie $d^x_{i,j}$, $d^y_{i,j}$ a $d^{xy}_{i,j}$ buï môu by dané všetky, alebo iba niektoré a z nich vieme vypoèíta hodnoty ostatnıch. 
...
Tento modifikovanı algoritmus má menej ne polovièné pamäové nároky ne v prípade všeobecnıch trojdiagonálnych sústav, èo nám v praxi umoòuje rieši väèšie úlohy. 
...
Ako bolo spomenuté, kadı segment splajnu potrebuje na svoje zostrojenie štyri uzly a hodnoty $z$, $d^x$, $d^y$ a $d^{xy}$. 
...
Teda máme dané $(u_0$, ...,$u_{I-1})$ a $(v_0$, ...,$v_{J-1})$, kde $I$,$J$ sú z \uinteger, prièom chceme zostroji splajn $S$, ktorı interpoluje hodnoty $\{z_{0,0}, \dots, z_{I-1,0}, \dots, z_{0,J-1} \dots z_{I-1,J-1}\}$ aby pre kadé $i \in \{0, \dots, I-1\}$, $j \in \{0, \dots, J-1\}$ platilo $z_{i,j} = S(u_i, v_j)$.
...
Pre lepšie pochopenie redukovaného algoritmu uvádzame grafické ilustrácie krokov pre vıpoèet restov, tzn. derivácií pomocou explicitnıch rovníc (\ref{eq:redr1}), (\ref{eq:redr2}), (\ref{eq:redr4a}), (\ref{eq:redr4b}) a (\ref{eq:redr4c}) znázornenıch na nasledujúcich obrázkoch. 
...
\begin{itemize}
	...
	\item
	pre kadé $i$ z $\{0, I-1\}$ a pre kadé $j$ z $\{0, J-1\}$ sú $d^{xy}_{i,j}$ zmiešané derivácie na osiach $x$ a $y$.
\end{itemize}
...
\begin{itemize}
	...
	\item
	$d^{xy}_{i,j}$, pre kadé $i$ z $\{0, \dots, I-1\}$ a pre kadé $j$ z $\{0, \dots, J-1\}$ okrem $d^{xy}_{0,0}$, $d^{xy}_{I-1,0}$, $d^{xy}_{0,J-1}$ a $d^{xy}_{I-1,J-1}$.
\end{itemize}
...
Všimnime si, e v prípade oboch algoritmov derivácie pod¾a jednotlivıch premennıch poèítame v vdy jednom smere.
...
Pamäové operácie sú pritom v závislosti od konkrétneho typu operaènej pamäte pribline dvadsanásobne pomalšie ako sèítanie, resp. násobenie.\footnote{V praxi sa aj vıpoèty rádovo tisícov uzlov èasto zmestia do vyrovnávacej pamäte procesora (\pojem{cache}). V tom prípade sú pamäové operácie len $1\times$-$10\times$ pomalšie ako sèítanie.}  
...
Toti ak jeden prvok matice má $8$ bajtov (ve¾kos èíselného typu \textit{double} vo väèšine jazykoch), tak matica zaberá v pamäti pribline $7\text{,}63$ mebibajtov. 
...
Obrázok \ref{img:cacheSpeed} nišie názorne ukazuje vzah medzi ve¾kosou dátovej štruktúry v KiB a poètom strojovıch cyklov, ktoré procesor musí èaka pre prístup k jej prvkom. 
...
\pozn{
	...
	Vo vıpoètoch typu $a[i] = a[i] \circ a[i-1]$ sa pipelining nedá poui, lebo v tomto prípade procesor nemôe zaèa vıpoèet $i$-ho prvku ak ešte nevypoèítal hodnotu $(i-1)$-teho prvku.
...
Z porovnania druhého a tretieho ståpca tabu¾ky \ref{tab:instr_speed} a druhého a tretieho ståpca tabu¾ky \ref{real_op_speed} môeme konštatova, e sèítavanie a násobenie môeme povaova za podobne rıchle operácie. 
...
\begin{pozn}
	...
	Ïalej pod¾a tabu¾ky \ref{real_op_speed} budeme súhrne uvaova $\gamma^\times = \rfrac{5}{4}$ $\gamma^\div = 3$, $\gamma^\rightY = \rfrac{1}{25}$ a $\gamma^\righttherefore = \rfrac{3}{5}$. Pri presnıch meraní zrıchlenia na konkrétnych procesoroch budeme samozrejme uvaova presné hodnoty pomerov pod¾a nameranıch hodnôt v predchádzajúcej tabu¾ke.
\end{pozn}
...
\begin{df}
	...
	\begin{itemize}
		...
		vzahom 
		$$c(\langle p^\pm, p^\times, p^\div, p^\rightY, p^\righttherefore \rangle) = \left\langle \ceil*{\frac{1}{\beta}p^\pm}, \ceil*{\frac{1}{\beta}p^\times \gamma^\times}, \ceil*{\gamma^\div p^\div},  \ceil*{\gamma^\rightY p^\rightY},  \ceil*{\gamma^\righttherefore p^\righttherefore} \right\rangle\text{,}$$
...
V kontexte poèítania operácií a cien budeme z dôvodu zjednodušenia procedúry povaova za mnoiny matematickıch vırazov, prièom algoritmy budeme povaova za mnoiny obsahujúce procedúry a vırazy.
...
\label{sec:full_cost}
Ceny základnıch aritmetickıch a pamäovıch operácií pre vyššie uvedené procedúry môeme zhrnú do tabu¾ky, prièom uvaujeme poèty v našej ukákovej implementácii\footnote{Repozitár so zdrojovımi kódmi k aplikáciám mono nájs na adrese https://github.com/vildibald/VKDiplom-master}.
...
Ïalej polome dve nové pomocné procedúry \textit{InicalizujZmiešLU}, ktorá inicializuje hodnoty $r_0$, \dots, $r_{K-1}$ a $b$ z LU dekompozície \ref{eq:LU} pre zmiešané derivácie $d^{xy}$ a k nej príslušnú \textit{VyriešZmiešLU}.
...
Cena kopírovania bude pod¾a reálneho merania v závislosti od konkrétneho systému pribline $0\text{,}7$.
...
Samotné uzly si pamäta nemusíme lebo uvaujeme len uniformné splajny. 
Teda staèí si nám uloi iba uzly $u_0$, $u_{I-1}$, $v_0$, $v_{J-1}$. 
...
\begin{itemize}
	...
	\item
	$\frac{25}{4}IJ + \frac{57}{2\beta}IJ + \frac{5}{4}\cdot \gamma^\div IJ + \frac{3}{2}\gamma^\righttherefore IJ$ pre redukovanı algoritmus.
\end{itemize}
...
Keïe nemáme k dispozícii procesor s vhodnou hardvérovou vıbavou, ktorı by mal viac ako dve jednotky pre vıpoèty s pohyblivou desatinnou èiarkou (ak vôbec takı existuje) je nároèné predpoklada ako by si takıto stroj s redukovanım postupom poradil. 
...
Toto rozdelenie vıpoètu na dve subprocedúry môe hypoteticky implikova vysokú réiu plánovaèa vlákien (tzv. \pojem{thread scheduler}) vrstvy OpenMP alebo operaèného systému. 
V implementácií sa nám podarilo spoji vıpoèet restov spolu s LU dekompozíciou do jedného kroku v prípade derivácií pod¾a premennıch $x$ a $y$ a resty tak môeme poèíta súèasne s deriváciami poèítanımi cez LU.
...
Druhı fakt spoèíva v takzvanom inštrukènom paralelizme. Procesorové jadrá zvyèajne obsahujú práve dve autonómne numerické koprocesory pre vıpoèty s plávajúcou desatinnou èiarkou, ktoré sú schopné matematické vırazy tvorené viac ako jednou operáciu sèítania alebo násobenia poèíta spoloène. Keïe redukovanı algoritmus obsahuje na rozdiel od plného algoritmu dlhšie vırazy s viacero operátormi $\plus$, $\minus$, $\cdot$ tak vïaka inštrukènému paralelizmu je cena vyhodnotenia vırazov polovièná vzh¾adom na poèet tıchto operátorov. 
...
To by v budúcnosti umonilo vıpoèty akcelerova pouitím masívne paralelizovanıch èipoch ako sú napríklad grafické karty. 

\part*{Príloha}
\chapter*{Implementácia a uívate¾ská príruèka} \label{chapt:impl}

Jednım z našich cie¾ov je vytvorenie aplikácií na vizuálne a vıkonnostné porovnávanie splajnov, ktorıch uzly sú poèítané De Boorovim a našim redukovanım algoritmom. 
...
Do spodného textového políèka oznaèené \textit{Expression} môeme napísa takmer ¾ubovo¾nú matematickú funkciu premennıch $x$ a $y$ ktorú si eláme vykresli. 
...
Poloka \textit{Settings} umonuje prispôsobi grafické vykres¾ovanie splajnov. 
Tieto nastavenia je vhodné upravi najmä pri starších poèítaèoch na dosiahnutie optimálnej plynulosti vykres¾ovania.
V strednej èasti panela máme k dispozícii vysúvate¾nı zoznam vykres¾ovanıch plôch, ktorım ich môeme zvırazòova. 
...
Vıznam jednotlivıch poloiek si teraz vyjasníme.
 \begin{itemize}
 	\item[1.]
 	\textit{Instructions benchmark} spustí celkovo tri testy. V teste \textit{Memory copy loop} pomerá rıchlos kopírovania  spojitıch (\textit{Continuous} - $\rightY$) a nespojitıch (\textit{Uncontinuous} - $\righttherefore$) dát. 
 	...
 	Test vyaduje postupné zadanie dvoj údajov analogicky ako na obrázku \ref{bench_surface}. 
 	Prvı udáva poèet opakovaní testu pre kadı algoritmus. 
 	Vıslednı pomer vznikne vydelením aritmetickıch priemerov nameranıch èasov pre kadı algoritmus (pozri obrázok \ref{bench_surface_results}). 
 	...
 	\item[4.]
	\textit{Spline surface benchmark (in parallel)} paralelná verzia testu pre plochy vyuívajúca na vıpoèet jednej matice uzlov všetky dostupné logické procesory.
% 	\item[B.]
