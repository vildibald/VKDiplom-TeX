...
\begin{pozn}
	...
	Ïalej pod¾a tabu¾ky \ref{real_op_speed} budeme súhrne uvaova $\gamma^\times = 1$ $\gamma^\div = 3$, $\gamma^\rightY = \rfrac{1}{25}$ a $\gamma^\righttherefore = 1$. Pri presnıch meraní zrıchlenia na konkrétnych procesoroch budeme samozrejme uvaova presné hodnoty pomerov pod¾a nameranıch hodnôt v predchádzajúcej tabu¾ke.
\end{pozn}
...
\pr{ \label{pr:ratio2}
	Nech $\beta = 2$, teda procesorové jadro má práve dve jednotky pre vıpoèty s pohyblivou desatinnou èiarkou, $\gamma^\times = 1$ a $\gamma^\div = 3$.
...
\pr{ \label{pr:ratio1}
	Nech $\beta = 1$, teda procesorové jadro má iba jednu jednotku pre vıpoèty s pohyblivou desatinnou èiarkou, $\gamma^\times = 1$ a $\gamma^\div = 3$, to jest operácia delenia je trojnásobne pomalšia ako operácia sèítania pod¾a tabuliek \ref{tab:instr_speed} a \ref{real_op_speed} operácií v èasti \ref{sec:instr_rychlost}. 
...
Môeme si všimnú malé škálovanie vıkonu paralelizovaného algoritmu najmä v prípade procesorov Intel obsahujúcich SMT kde je redukovanı algoritmus dokonca o nieèo pomalší ako plnı. 
