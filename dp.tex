...
Kım v prípade splajnovıch kriviek bolo evidentné hneï od zaèiatky ich vıskumu a aplikácie, e okrem hodnôt $z_0$, $z_1$, ..., $z_{I-1}$ je potrebné zada dve podmienky, v našom prípade dve hodnoty $d_0$ a $d_{I-1}$.
V prípade interpolaènıch splajnovıch povrchoch nebolo zrejmé aké podmienky je potrebné zada.
...
Úloha v následovnıch èastiach práce je na základe vstupnıch uzlov $u_{0}$, ..., $u_{I-1}$, $v_{0}$, ..., $v_{J-1}$ a funkènıch hodnôt $z_{0,0}$, ..., $z_{I,J}$ nájs \pojem{hladkú}, po èastiach definovanú funkciu $S: [u_0, u_{I+1}] \times [v_0, v_{J+1}] \to \mathbb{R}$ so spojitımi deriváciami prvého aj druhého rádu takú, e pre kadé $i \in  {0, \dots, I-1}$ a $j \in  {0, \dots, J-1}$ platí $z_{i,j} = S(u_i, v_j)$. 
...
Na vypoèítanie kadého segmentu potrebujeme štyri uzly, a pre kadı uzol príslušne hodnoty $z$, $d^x$, $d^y$ a $d^{xy}$. 
...
Predtım popíšeme technické parametre procesorov v dôsledku ktorıch dosahujeme zrıchlenie. 
Tu iba poznamenajme, e zrıchlenie nastáva vïaka zredukovanému poètu delení.
Keïe porovnávame asymptoticky rovnako rıchle algoritmy, musíme bra do úvahy vplyv \pojem{nízkoúrovòovıch} operácií, 
konkrétne rıchlosti vıpoètu sèítania, násobenia, delenia a na koniec aj samotnı vplyv pamäovıch operácií èítania a zapisovania. 
...
Pamäové operácie sú pritom v závislosti od konkrétneho typu operaènej pamäte pribline dvadsanásobne pomalšie ako sèítanie, resp. násobenie.\footnote{V praxi sa aj vıpoèty rádovo tisícov uzlov èasto zmestia do vyrovnávacej pamäte procesora (\pojem{cache}). 
	V tom prípade sú pamäové operácie len $1\times$-$10\times$ pomalšie ako sèítanie.}  
...
Moderné procesorové mikroarchitektúry èasto pouívajú rôzne optimalizaèné \uv{triky a finty} ako dosiahnu zlepšenie vıkonu a ktoré majú znaènı vplyv na reálne rıchlosti algoritmov. 
...
tieto jednotky vo všeobecnosti nedokáu fungova samostatne a simultánne spracováva viacero vlákien alebo procesov\footnote{Existujú technológie ako napríklad Hyper-threading umoòujúce za urèitıch podmienok uplatni vláknovı paralelizmus aj v rámci jediného superskalárneho jadra.}. 
...
V opaènom prípade dochádza k javu kedy jeden proces (vlákno) musí èaka na uvo¾nenie jednotky druhám procesom (vláknom) a vtedy môe dôjs k zníeniu vıkonu oproti identickému procesor s neaktívnym SMT.
...
Definujme funkciu $o^\mathbb{V}: \mathbb{V} \to \mathbb{N} \times \mathbb{N} \times \mathbb{N}$ vzahom 
...
\begin{pozn}
Faktor superskalárnosti $\beta$ budeme uvaova iba pre operácie sèítania násobenia. 
...
\begin{df}
	...
	\begin{itemize}
		...
		ktorú nazveme \pojem{cena operácií vırazu $V$}
	\end{itemize}
\end{df} 
...
%			& $12IJ$ & $9IJ$ & $9IJ$ & $3IJ$\\
		...
		\end{tabular}
	\end{center}
	\caption{Poèty operácií plného algoritmu.}
...
\begin{center}
	...
	\end{tabular}
\end{center}
\caption{Poèty operácií LU dekompozície pre redukovanı algoritmus.}
...
\begin{center}
	...
	\end{tabular}
\end{center}
\caption{Poèty operácií LU dekompozície na spojitıch dátach pre zmiešané zostatkové derivácie.}
...
\begin{center}
	...
	\end{tabular}
\end{center}	
\caption{Poèty operácií na spojitıch dátach redukovaného algoritmu.}
...
\begin{table}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Poèty operácií pre zmiešané zostatkové derivácie.}
...
Prioritou teraz ostáva jednak zovšeobecni redukovanı algoritmus aj pre nerovnomerné uzly splajny a upravi zmiešané zbytkové derivácie aby sme mohli zefektívni paralelizáciu.
%Zlepšenie paralelizácie by v budúcnosti umonilo vıpoèty akcelerova grafickou kartou. 
Paralelné poèítanie uzlov redukovanım algoritmom je v súèasnosti pomalšie ako pri plnom algoritme.
Vıhodou ale ostávajú polovièné pamäové nároky èo umoòuje rieši väèšie úlohy a tak rozdeli prácu medzi viacero logickıch procesorov. 
