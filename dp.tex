...
Jedná sa o ve¾mi schopnı nástroj na tvorbu webovıch aplikácii s plnou podporou hardvérovej akcelerácie uívate¾ského prostredia a monosou tvorby rıchlej trojrozmernej grafiky. 
...
V praxi vidno, e sèítavanie a násobenie môeme povaova za podobne rıchle operácie. 
...
\begin{pozn}
	Pre moderné procesory architektúry x86 budeme pod¾a tabu¾ky \ref{real_instrpar_ratio} uvaova $\beta = 2$. 
	Ïalej pod¾a tabu¾ky \ref{real_op_speed} budeme súhrne uvaova $\gamma^\div = 3$, $\gamma^\rightY = \rfrac{1}{25}$ a $\gamma^\righttherefore = 1$.
\end{pozn}
...
\begin{center}
	\begin{tabular}{lcccc}	
		...
		\midrule
		\textit{InicalizujLU}
		& $\frac{3}{\beta}K$ & $\frac{2}{\beta}K$ & $0$ & $0$\\
		\textit{VyriešLU}
...
\begin{center}
	...
	\begin{tabular}{lcccc}	
		...
		\midrule
		\textit{VypoèítajDx}
		& $\frac{3}{2}IJ+\frac{3}{2\beta}IJ$ & $1IJ+\frac{1}{\beta}IJ$ & $\frac{1}{2}\gamma^\div IJ$ & $\frac{1}{2}\gamma^\righttherefore IJ$\\
		\textit{VypoèítajDy}
		& $\frac{3}{2}IJ+\frac{3}{2\beta}IJ$ & $1IJ+\frac{1}{\beta}IJ$ & $\frac{1}{2}\gamma^\div IJ$ & $\frac{1}{2}\gamma^\righttherefore IJ$\\
		\textit{VypoèítajDxy}
		& $8I+8J$ & $6I+6J$ & $2I+2J$ & $\gamma^\rightY I+\gamma^\rightY J$\\
		\textit{VypoèítajDyx}
		& $\frac{3}{4}IJ+\frac{33}{4\beta}IJ$ & $\frac{1}{2}IJ+\frac{17}{4\beta}IJ$ & $\frac{1}{4}\gamma^\div IJ$ & $\frac{1}{2}\gamma^\righttherefore $\\
		\bottomrule
	\end{tabular}
\end{center}	
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{lcccc}	
			...
			& $\frac{3}{2\beta}IJ$ & $\frac{1}{\beta}IJ$ & $0$ & $0$\\
			\textit{VypoèítajDyResty}
			& $\frac{3}{2\beta}IJ$ & $\frac{1}{\beta}IJ$ & $0$ & $0$\\
			\textit{VypoèítajZmiešResty}
			& $\frac{17}{4\beta}IJ$ & $\frac{7}{4\beta}IJ$ & $0$ & $0$\\
			\bottomrule
		\end{tabular}
	\end{center}
...
\begin{lema}
	...
	\begin{itemize} 
		...
		\item
		$\frac{5}{2}IJ + \frac{10}{\beta}IJ$ násobení,
		\item
	...
	$$\frac{25}{4}IJ + \frac{57}{2\beta}IJ + \frac{5}{4}\cdot \gamma^\div IJ + \frac{3}{2}\gamma^\righttherefore IJ\textit{.}$$
\end{lema}
...
Uvaujme rovnomernú obdånikovú sie uzlov. 
Oèakávané zrıchlenie sériového redukovaného algoritmu je
$$\frac{21+3\gamma^\div+3\gamma^\rightY}
	{\frac{25}{4} + \frac{57}{2\beta} + \frac{5}{4}\cdot \gamma^\div + \frac{3}{2}\gamma^\righttherefore}$$ 
\end{veta}
...
\pr{ \label{pr:ratio2}
	...
	Analogicky ako v predchádzajúcom príklade dosadením do vzahu dostaneme
...
\pr{ \label{pr:ratio1}
	...
	Dosadením do vzahu z predchádzajúcej vety dostaneme
...
Druhı fakt spoèíva v takzvanom inštrukènom paralelizme. Procesorové jadrá zvyèajne obsahujú práve dve autonómne numerické koprocesory pre vıpoèty s plávajúcou desatinnou èiarkou, ktoré sú schopné matematické vırazy tvorené viac ako jednou operáciu sèítania alebo násobenia poèíta spoloène. Keïe redukovanı algoritmus narozdiel obsahuje dlhé vırazy s viacero operátormi $\plus$, $\minus$, $\cdot$ tak vïaka inštrukènému paralelizmu je cena vyhodnotenia vırazov polovièná vzh¾adom na poèet tıchto operátorov. 
...
Na obrázku \ref{bench} môeme vidie úvodné okno aplikácie, ktoré obsahuje šes poloiek. 
Ich vıznam si teraz vyjasníme.
 \begin{itemize}
 	...
 	Ïalej pomerá rıchlos kopírovania spojitıch ($\rightY$) a nespojitıch ($\righttherefore$). 
 	Nakoniec zistí vplyv inštrukèného paralelizmu na tri vyššie uvedené aritmetické operácie, kde vısledné údaje sú pomery doby vyhodnotenia vırazov s desiatimi operátormi a doby vyhodnotenia vırazu s práve jednım operátorom. 
 	\item[2.] 
 	...
 	Pred spustením aplikácie vyaduje postupné zadanie dvoj údajov. 
 	Prvı udáva poèet opakovaní testu pre kadı algoritmus. 
 	Vıslednı pomer vznikne vydelením aritmetickıch priemerov nameranıch èasov pre kadı algoritmus. 
