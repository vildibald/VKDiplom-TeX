...
Merali sme operácie na 512 prvkovom poli, prièom aby sme dostali \uv{rozumne} dlhé èasy vıpoèty boli opakované 500000 krát.
...
To by procesoru umonilo poui techniku pipeliningu a pomery èasov násobenia a sèítania by vyšli, vïaka väèšiemu prietoku tıchto dvoch operácií oproti deleniu, násobne väèšie.
...
\item
Hodnota $\gamma^\div$ znaèí \pojem{pomer odozvy delenia a násobenia} pri operáciách s plávajúcou desatinnou èiarkou.
Inımi slovami hodnota $\gamma^\div$ znaèí ko¾kokrát je delenie pomalšie oproti sèítaniu v prípade, keï nedochádza k èastému opakovanému násobeniu (resp. sèítaniu).
\item
...
\end{oznac}
\begin{pozn}
	Pre moderné procesory architektúry x86 budeme uvaova $\beta = 2$, $\gamma^\div = 3$, $\gamma^\rightY = \rfrac{1}{25}$ a $\gamma^\righttherefore = 1$.
\end{pozn}
\begin{pozn}
...
\begin{df}
	...
	\begin{itemize}
		...
		vzahom 
		$$c(\langle s, n, d \rangle) = \left\langle \ceil*{\frac{s}{\beta}}, \ceil*{\frac{s}{\beta}}, \gamma^\div d \right\rangle\text{,}$$
...
\pr{ \label{pr:ratio2}
	...
	V prípade superskalárnosti s faktorom 2 je redukovanı algoritmus o 17\% rıchlejší ako plnı.
}
...
%Ståpce 2 a 4 predstavujú merané pomery rıchlostí $\gamma^\div$, $\gamma^\rightY$ a $\gamma^\righttherefore$ aritmetickıch operácií pod¾a tabu¾ky \ref{real_op_speed} v èasti \ref{sec:instr_rychlost}.
...
V následujúcej tabu¾ke budeme uvaova merané rıchlosti aritmetickıch operácií z tabu¾ky \ref{real_op_speed} v èasti \ref{sec:instr_rychlost}. 
...
\begin{table}[ht]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Porovnanie teoretického a meraného zrıchlenia na mrieke $1000\times 1000$ uzlov.}
...
Môeme si všimnú malé škálovanie vıkonu paralelizovaného algoritmu najmä v prípade procesorov obsahujúcich nejakú formu SMT kde je redukovanı algoritmus dokonca o nieèo pomalší ako plnı. 
...
Samotnı vıvoj aplikácii môe prebieha v ¾ubovo¾nom programovacom jazyku beiacom pod Common Language Runtime. 
...
Napriek tomu, e vıslednı redukovanı algoritmus obsahuje viac aritmetickıch operácií je vïaka povahe mikroarchitektúr modernıch procesosov a tie vïaka povahy samotnıch operácií rıchlejší o pribline 20\%.
