...
%\begin{itemize}
%\end{itemize}
%\item{\textbf{Pouívate¾ská príruèka}}\newline
%\begin{itemize}
%\end{itemize}
\item{\textbf{Záver}}\newline
\end{itemize}
...
\obrazok{jarr.png}{0.407}{Vizuálna reprezentácia dátovej štruktúry \uv{pole polí}.}
...
\obrazok{cacheIntel.png}{0.5}{Graf rıchlosti prístupu k dátam rôznych ve¾kostí na modernıch CPU\cite{cacheIntel}.}
...
\end{itemize}
Nech $I$ znaèí poèet uzlov na osi $x$ a $J$ znaèí poèet uzlov na osi $y$. Všetky poèty sú v tvare $a\cdot IJ + b\cdot I + c\cdot J + d$. Pre zjednodušenie budeme poèty, tam kde je koeficient $a$ nenulovı, uvádza v tvare $a\cdot IJ$. Keïe poèty rastú kvadraticky tak pre ve¾ké $I$ a $J$ bude odchılka zanedbate¾ná. Pre plnı algoritmus teda dostaneme poèty operácií nad kešovanımi dátami   
...
\begin{center}\label{red_lu_operations}
	...
	\end{tabular}
\end{center}
Podobne ako pri plnom algoritme, to èi operácie budú vykonané na kešovanıch dátach závisí od toho z ktorej procedúry sú volané. 
Pri procedúrach \textit{InicalizujZmiešLU} a \textit{VyriešZmiešLU}, ktoré sú volané len v jednom kontexte ale vieme exaktne urèi ktoré operácie budú vykonané na kešovanıch dátach a ktoré nie.
...
Najprv si spoèítajme procedúry pre resty. Všimnime si, e všetky vıpoèty v \textit{VypoèítajDyResty} sú kešované ale vıpoèty v \textit{VypoèítajDxResty} nie.
...
\begin{center}\label{rest_operations_cached}
	...
	\end{tabular}
\end{center}
Pri nekešované vıpoèty v \textit{VypoèítajDxResty} dostávame
\begin{center}\label{rest_operations_uncached}
	\begin{tabular}{l|cccc}
		\diag{.1em}{4cm}{Procedúra}{Operácia}
		& $+$ & $\times$ & $\div$ & M \\
		\hline
...
\begin{center}\label{red_operations_uncached}	
	\begin{tabularx}{\textwidth}{l|XXXX}
		...
		\hline
		\textit{VypoèítajDx}
		& $1\textit{,}5IJ$ & $IJ$ & $0\textit{,}5IJ$ & $0\textit{,}5IJ$\\
		\textit{VypoèítajDy}
		& $0$ & $0$ & $0$ & $0$\\
		\textit{VypoèítajDxy}
		& $2I$ & $2I$ & $2J$ & $J$\\
...
\end{df}
\pr{ \label{ratio10}
	Ak budeme predpoklada $\gamma = 10$, to jest operácia delenia je desanásobne pomalšia ako operácia sèítania tak a budeme uvaova maticu uzlov s poètom $I\times J = 1000\times1000$ tak pomer rıchlostí bude $1{,}187$.	
}
